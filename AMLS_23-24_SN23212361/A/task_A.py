...
from A.models import ResNet18
from A.dataset import INFO, taskA
from A.evaluator import getAUC, getACC, save_results


def mainA(flag, input_root, output_root, end_epoch):

    dataclass = {
        "PneumoniaMNIST": taskA
    }
    ...
    test(model, 'test', test_loader, device, flag, output_root=output_root)##我
...
def run_A():
    ...
    parser.add_argument('--data_name', default='PneumoniaMNIST', help='subset of MedMNIST', type=str)
    parser.add_argument('--input_root', default='./Datasets', help='input root, the source of dataset files', type=str)
    parser.add_argument('--output_root', default='./A/output_A', help='output root, where to save models and results',
                        type=str)
    ...
    mainA(data_name, input_root, output_root, end_epoch=end_epoch)
